...
- 2026-05-26: Manual retest rejected the previous level-end stale-bank
  hypothesis. Current screenshot `/Users/magnus-bot/Desktop/bug fine.png`
  shows a different stale-MO case: interactive high-score initials entry after
  the final race is active (`main=1`, `mode=2`, draw-list empty), the playfield
  is correctly cleared, but old motion-object RAM still renders blue garbage
  rows and a stale marble. Fix: when the `FUN_11B18` fallback starts the
  interactive initials entry, clear `spriteRam` together with `playfieldRam`
  before re-rendering the high-score table. Focused validation passed:
  `npx vitest run packages/engine/test/main-loop-init-task-a.test.ts packages/engine/test/object-slot-lookup-11b18.test.ts --silent`;
  `npx vitest run packages/engine/test/main-tick.test.ts packages/engine/test/main-loop-init-task-a.test.ts packages/engine/test/object-slot-lookup-11b18.test.ts --silent`;
  `npx tsc -p packages/engine/tsconfig.json --noEmit --pretty false`;
  `npm --workspace @marble-love/engine run build`;
  `npx tsc -p packages/web/tsconfig.json --noEmit --pretty false`;
  `npm --workspace @marble-love/web run build` (known chunk-size warning);
  `git diff --check`. Manual browser retest remains pending.
