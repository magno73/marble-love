...
- Il path audio `soundReplay` ha una baseline oracle utile: YM/POKEY write
  parity e PCM audible-window gate sono verdi sul preset corrente con
  tolleranza nativa `+-1`, ma la parita' bit-perfect esatta resta aperta su
  timing/scheduler. Non usare diagnostiche audio per guidare modifiche
  gameplay.
