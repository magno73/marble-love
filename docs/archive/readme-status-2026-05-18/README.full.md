...
non era un seed sbagliato: MAME mantiene stabile la surface player `0x401C28`
nel post-seed L5, mentre TS forzava `FUN_1CABA` e cancellava il pavimento. La
guardia player-scoped su descriptor L5 `0x2de1e` rende active e neutral exact
contro MAME per 180 frame e lo smoke ROM-backed resta stabile per 120 tick.
Il seed resta candidato non cablato finche' non passa il gate completo di
wiring/manual review.
...
abbastanza controllabile. Sul caso L5 f3520 lo stesso confronto ha dimostrato
...
e' cablato ai candidati descriptor reali; `?autoLoad=1&play=1` parte dal gate
attract/coin/start e, dopo START, usa il true-start L1
`start_level1_intro_practice_f2479`, mentre `manual_level1_start` resta un seed
legacy esplicito via `?playableSeed=manual_level1_start`.
...
(`0x2c54c`) e resta solo come seed legacy esplicito via
`?playableSeed=manual_level1_start`.
...
| **Live browser input** | ✅ `?autoLoad=1&play=1` parte dal gate attract/coin/start e dopo START carica il true-start L1 `start_level1_intro_practice_f2479` con banner/timer MAME e trackball neutro; `?autoLoad=1&coinStart=1` mantiene lo stesso gate esplicito; `manual_level1_start` resta legacy esplicito via `?playableSeed=manual_level1_start`; `coin_start_to_level1` resta replay/oracle fallback; `?preserveDispatcher=1` conserva invece il dispatcher MAME per drill oracle; trace/replay conserva la rotazione MAME trackball, mentre mouse/touch/WASD/frecce/gamepad live usano assi screen-space mono-asse con X invertito per il controllo visivo; frecce libere dal debug-scroll in `?play=1`; lower-platform death/respawn state-1 e route live browser coperte da regression test; seed playable web via `?playableSeed=...` solo per diagnostica |
...
# 2. Premi 5 (coin) + Enter/spazio (START1)
# 3. Il true-start L1 mostra il banner/timer iniziale, poi entra in gameplay
# 4. Muovi con mouse / WASD / frecce. I comandi sound gameplay producono cue udibili.
# Usa ?autoLoad=1&coinStart=1 se vuoi lo stesso gate senza implicare play=1.
