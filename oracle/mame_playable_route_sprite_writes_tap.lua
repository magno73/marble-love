-- mame_playable_route_sprite_writes_tap.lua
--
-- Wrapper for oracle/mame_playable_input_capture.lua that also records
-- motion-object RAM writes during a routed playable capture.
...
local FROM_FR = getenv_num("MARBLE_TRACE_FROM", 3600)
local TO_FR = getenv_num("MARBLE_TRACE_TO", 3700)
local LO = getenv_num("MARBLE_TRACE_LO", 0xA02000)
local HI = getenv_num("MARBLE_TRACE_HI", 0xA023FF)
local OUT_PATH = os.getenv("MARBLE_TRACE_OUT") or "/tmp/mame_playable_sprite_writes_tap.json"
...
local function install()
    mem:install_write_tap(LO, HI, "playable_route_sprite_writes_tap", function(offset, data, mask)
...
emu.register_frame_done(function()
    ...
    if (not installed) and frame_count >= FROM_FR then
        install()
        installed = true
        print(string.format("[playable_route_sprite_writes_tap] installed 0x%06X..0x%06X at frame %d", LO, HI, frame_count))
    end

    if frame_count >= TO_FR then
        write_json()
        print(string.format("[playable_route_sprite_writes_tap] DONE writes=%d -> %s", writes, OUT_PATH))
    end
end)
