...
--   MARBLE_TRACE_OUT  — output JSON (default /tmp/mame_cluster_0706.json)
...
local LO        = tonumber(getenv("MARBLE_TRACE_LO", "0x400706"))
local HI        = tonumber(getenv("MARBLE_TRACE_HI", "0x400751"))
local FROM_FR   = tonumber(getenv("MARBLE_TRACE_FROM", "11998"))
local TO_FR     = tonumber(getenv("MARBLE_TRACE_TO", "12099"))
local OUT_PATH  = getenv("MARBLE_TRACE_OUT", "/tmp/mame_cluster_0706.json")
...
local function install()
    mem:install_write_tap(LO, HI, "cluster_0706", function(o, d, m)
        writes = writes + 1
...
emu.register_frame_done(function()
    ...
    if frame_count == FROM_FR - 1 and not installed then
        install()
        installed = true
        print(string.format("[cluster_0706] tap on 0x%X..0x%X frames=%d..%d",
            LO, HI, FROM_FR, TO_FR))
    end
    ...
    if frame_count % 2000 == 0 then
        print(string.format("[cluster_0706] fc=%d writes=%d", frame_count, writes))
    end
    ...
    if frame_count > TO_FR then
        ...
        f:write("}\n")
        f:close()
        print(string.format("[cluster_0706] DONE writes=%d distinct_pcs=%d -> %s",
            writes, #pcs, OUT_PATH))
