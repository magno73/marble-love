/**
 *
 * word quotient zero-extended to long.
 *
 *   - valid unsigned long (0..0xFFFFFFFF typical, but in the marble-madness caller
 *     they are typically in the word/byte range)
 *   - `-1` (= 0xFFFFFFFF) or `-2` (= 0xFFFFFFFE) for OOR errors
 *
 * and for parity (the binary oracle patches FUN_40D8 entry with synthetic RTS
 * and injects D0 manually).
 *
 * **Disasm 0x59D2..0x5A5D** (140 bytes / 0x8C):
 *
 ...
 *
 * **Caller convention** (single xref @ 0x5B8E in FUN_5A5E):
 *   - D2 callee-saved (prologue/epilogue preserve it).
 *   - Return: long unsigned in D0 (= word quotient, range 0..0xFFFF, or 0
 ...
 *
 *   1. **`asl.l #1, D2`**: arithmetic shift left by 1 = multiplication by 2,
 *      equivalent to `add.l D2, D2`. Sets CCR.X, CCR.N, CCR.Z, CCR.V (V on overflow), CCR.C.
 *      wraps mod 2^32 (long arithmetic semantics). Model this as unsigned32.
 *
 *   2. **`add.l D0, D2`**: long add. Wrap mod 2^32. CCR.X/N/Z/V/C set.
 ...
 *      `cmpi.l #0xFFFF,D2; addq.l #4,SP; bhi`: the `addq #4,SP` does not clobber
 *      the cmpi flags.
 *
 ...
 *          0x5A24: cmpi.l #0x1FFFE, D1
 *          0x5A2A: bls.b 0x5A3A   ; if D1 <= 0x1FFFE → branch to ROUND-half
 ...
 *
 *   8. **`mulu.w #0x3C, D0`**: D0w * 60 → long in D0. Does NOT overflow the long
 *      (max 0xFFFF * 60 = 0x3BFFC4 < 2^32). Safe.
 *
 ...
 * only for tests.
 *
 * @returns       long unsigned (D0 at the callee's rts).
 */
 ...
 *
 * 2. The halving step `(x + 1) >> 1` models `addq.l #1, D0; lsr.l #1, D0`.
 *
 * 3. The halve-loop has its entry @ 0x5A1A. Re-check on D2 and D1. Fall through to 0x5A4A
 *    through the ROUND-half path (NOT through the LSR path, which has explicit `bra 0x5A1A`).
 *    ONLY round-half ((D2+1)>>1, (D1+1)>>1), then divu.
 *
 *    D2 = 0xFFFF <= 0x1FFFE → no LSR. D1 = 0x10000 <= 0x1FFFE → bls → ROUND-half.
 *    D2' = 0x10000>>1 = 0x8000; D1' = 0x10001>>1 = 0x8000. Proceeds to divu.
 *
 ...
 *
 * 4. `divu.w D2, D1`: unsigned word division. Quotient in the low word of D1,
 *    remainder in the high word. If quotient > 0xFFFF → V flag, D1 unchanged.
 *    In TS: detection via `Math.floor(num / denomW) > 0xFFFF`.
 *
 *    (exact 32-bit copy).
 *
 *    For overflow detection: `D1_pre_divu = (D1_post_halving & 0xFFFF) * 60`.
 *    Theoretical quotient = `Math.floor(D1_pre_divu / (D2 & 0xFFFF))`. If > 0xFFFF
...
export function stateSub59D2(
  ...
  // ─── Phase 5: bypass-fits-in-word vs halve-loop ──────────────────────────
  ...
  if (d2 > WORD_MAX || d1 > WORD_MAX) {
    ...
    //   D2 = D2 >>> 1; D1 = D1 >>> 1
    // then ONE ROUND-half: D2 = (D2+1)>>1; D1 = (D1+1)>>1
    // then falls into divu.
    //
  ...
  // ─── Phase 6: D0 = (D1 & 0xFFFF) * 60; D1 = D0 (long) ────────────────────
  ...
  // ─── Phase 7: divu.w D2, D1 ──────────────────────────────────────────────
  // Quotient = floor(D1 / D2w). If > 0xFFFF -> V flag, D1 unchanged.
  const d2Word = d2 & 0xffff;
  //
  // directly. d2 != 0 guaranteed by the early-exit path.
  //
  let d1AfterDivu: number;
  if (d2Word === 0) {
    // Theoretically unreachable path, but for safety we treat it as V-flag
  ...
  // ─── Phase 8: D0 = D1 low word zero-ext ──────────────────────────────────
