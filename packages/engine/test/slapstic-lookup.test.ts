 ...
 *
 * Bit-perfect parity verified against the binary in `test-slapstic-lookup-parity.ts`.
 *
...
describe("slapsticLookup (FUN_2FFB8)", () => {
  ...
  it("arg with bit 10 set (0x400): (arg<<5)&0xFFFF = 0x8000 → signExt16 → -0x8000 → addr 0x78080 (main ROM)", () => {
    const rom = makeRom();
  ...
  it("argW beyond 16 bits is masked to 16 bits", () => {
    const rom = makeRom();
