...
describe("soundStatusCheck (FUN_4C3E)", () => {
  it("soundPending=true → returns 0 and non tocca nulla", () => {
  ...
  it("slot occupied (owner long != 0) → returns 0 and non tocca nulla", () => {
    const s = emptyGameState();
    ...
    s.workRam[0x1f44 + 0x14] = 0xaa; // type byte non must cambiare
  ...
  it("type byte = (D0 >> 16) & 0xFF — D0 alto outside dagli 8 bit bassi", () => {
    const s = emptyGameState();
  ...
  it("ordering check: pending DOMINA su slot free → 0", () => {
    const s = emptyGameState();
    const r = soundStatusCheck(s, 0x10003, 0x401f44, true);
    expect(r).toBe(0);
  ...
  it("owner long also solo 1 byte set != 0 → fail", () => {
    const s = emptyGameState();
