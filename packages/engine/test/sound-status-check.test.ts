/**
 * sound-status-check.test.ts — corner cases of soundStatusCheck (FUN_4C3E).
 *
 */
...
describe("soundStatusCheck (FUN_4C3E)", () => {
  it("soundPending=true → returns 0 and touches nothing", () => {
  ...
  it("slot occupied (owner long != 0) → returns 0 and touches nothing", () => {
    const s = emptyGameState();
    ...
    s.workRam[0x1f44 + 0x14] = 0xaa; // type byte must not change
  ...
  it("slot free, no pending → returns 1, writes type byte + owner long", () => {
    const s = emptyGameState();
    const r = soundStatusCheck(s, 0x10003, 0x401f44, false);
    expect(r).toBe(1);
  ...
  it("type byte = (D0 >> 16) & 0xFF — D0 high outside the low 8 bits", () => {
    const s = emptyGameState();
  ...
  it("ordering check: pending DOMINATES over a free slot → 0", () => {
    const s = emptyGameState();
    const r = soundStatusCheck(s, 0x10003, 0x401f44, true);
    expect(r).toBe(0);
  ...
  it("owner long with even just 1 byte set != 0 → fail", () => {
    const s = emptyGameState();
