/**
 * slapstic-103.test.ts — verifica state machine chip 137412-103 con scenari
 ...
 * per ogni branch (idle/active/alt/bit), non solo riproducono behavior.
 */
...
describe("slapstic-103 — verified against MAME trace (140 accesses, f12000-12005)", () => {
  ...
  // Bank atteso dopo f=12000: 1 (verificato via data-match contro rom blob).
  // Bank atteso dopo f=12001: 1 (con transizioni 1→2→1 nel mezzo via lookups).
...
describe("slapstic-103 — alt sequence does NOT inadvertently break on normal reads", () => {
  it("alt1 trigger followed by non-alt2 read goes back to ACTIVE", () => {
    ...
    // Pero' un valore "0x5A" sul bus puo' essere triggerato? In pratica MAME
