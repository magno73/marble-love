/**
 * sort-adjacent-objects-1a7a8.test.ts — smoke test di FUN_1A7A8.
 *
 * Bit-perfect parity vs binario in `test-sort-adjacent-objects-1a7a8-parity.ts`.
 * Qui copriamo i path principali della logica di walk + swap + lookup ROM +
 * compare inline, senza Musashi.
 */
...
/** Scrive un long BE in un Uint8Array a un offset. */
...
/** Scrive una word BE (signed) in workRam a un absolute address M68k. */
...
/**
 * Helper: setup ROM lookup con N entry, ognuna pointer a un slot rect in
 * workRam @ 0x4001DC + i*0x10 (struct 14 byte ma stride 16 per comodità).
 */
...
/**
 * Helper: scrive un "rect" 14-byte struct con tutti i campi a `value` come
 * word BE signed. Utile per produrre un compare deterministico.
 */
function writeRect(state: GameState, abs: number, value: number): void {
  // Solo offsets +2,+4,+6,+8,+A,+C usati da fun1A80A.
...
describe("fun1A80A — rect compare (FUN_1A80A)", () => {
  ...
  it("D3 > D2, D5 <= D4 → return 1", () => {
    ...
    writeRect(s, 0x401000, 0); // tutti 0 → D2=0, D5=0 inizialmente
  ...
  it("Word compare path: tutti uguali → ritorna 1 (last fallthrough)", () => {
    const s = emptyGameState();
    // A1 e A0 con stesso pattern per saltare i 4 cmp.w iniziali e cadere
    // nel `moveq #1`. Setup: D3 > D2, D5 > D4 (entrambi non scattano).
    // Esempio: A1: tutti 1. A0: tutti 0.
    // D2 = 0, D3 = 3 → D3 > D2 ok; D4 = 3, D5 = 0 → D5 <= D4 → return 1.
    // Devo invertire: A0 con sumY più alto.
...
describe("lookupRectPtr — ROM table @ 0x1F0E2", () => {
  ...
  it("read pointer @ idx 255 (fuori i 16 entry validi)", () => {
    const rom = emptyRomImage();
...
describe("sortAdjacentObjects1A7A8 — single-pass walk", () => {
  it("Array tutti 0xFF → exit immediato, no mutation", () => {
    const s = emptyGameState();
    const rom = emptyRomImage();
  ...
  it("Compare sempre 0 (no swap) → walk fino a 0xFF, no mutation", () => {
    const s = emptyGameState();
    const rom = emptyRomImage();
    ...
    setupLookup(rom, 5);
    // Tutti i rect uguali → fun1A80A ritorna 0 (D3 == D2 → D3 <= D2 → 0).
  ...
  it("Compare sempre 1 (always swap) stride=1 → swap di tutte le coppie adiacenti", () => {
    const s = emptyGameState();
    const rom = emptyRomImage();
    // Array: [0, 1, 2, 3, 4, 0xFF].
    ...
    // Custom compare che ritorna sempre 1.
    sortAdjacentObjects1A7A8(s, rom, 1, {
      compare: () => 1,
    });

    // Walk con stride=1 swap-always: comportamento "rotate left by 1" finché
    // non vede 0xFF.
    //   step0: A2=3BC byte[3BC]=0, A3=3BD byte[3BD]=1. byte[3BD]!=FF.
    //          swap → array = [1,0,2,3,4,FF]. A2=3BD, A3=3BE.
  ...
  it("Compare sempre 1 stride=2 → swap su pairs distanti 2", () => {
    const s = emptyGameState();
    const rom = emptyRomImage();
    s.workRam[BYTE_ARRAY_OFF + 0] = 0;
    s.workRam[BYTE_ARRAY_OFF + 1] = 1;
    ...
    //        wait, byte[3BD]=1, byte[3BF]=3. swap dei due → [2,3,0,1,4,FF].
    //        Hmm meglio rifare a mano: dopo step0, array = [2,1,0,3,4,FF].
  ...
  it("compare() callback riceve i pointer ROM-resolved corretti", () => {
    const s = emptyGameState();
    const rom = emptyRomImage();
    ...
    // Solo 1 iter (poi byte[A3] == 0xFF → exit dopo addq).
  ...
  it("stride=0 con compare=1 → A2==A3, swap no-op (byte=byte)", () => {
    ...
    // Con stride=0 A2==A3, swap di byte con se stesso = no-op.
    expect(s.workRam).toEqual(before);
  });

  it("Mutation NON tocca byte fuori 0x3BC..0x3DC", () => {
    const s = emptyGameState();
    const rom = emptyRomImage();
    s.workRam[BYTE_ARRAY_OFF + 0] = 0;
    s.workRam[BYTE_ARRAY_OFF + 1] = 1;
    s.workRam[BYTE_ARRAY_OFF + 2] = 0xff;
    // Mark sentinel fuori range
  ...
  it("Pure: rect struct in workRam @ 0x1DC.. non vengono modificate", () => {
    const s = emptyGameState();
    const rom = emptyRomImage();
    s.workRam[BYTE_ARRAY_OFF + 0] = 0;
    s.workRam[BYTE_ARRAY_OFF + 1] = 1;
