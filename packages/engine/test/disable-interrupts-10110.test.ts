...
describe("disableInterrupts10110 (FUN_00010110, 6 bytes)", () => {
  it("smoke: returns 0x2700 (SR_IPL7_SUPERVISOR)", () => {
    expect(disableInterrupts10110()).toBe(0x2700);
  });

  it("constant SR_IPL7_SUPERVISOR is 0x2700", () => {
    expect(SR_IPL7_SUPERVISOR).toBe(0x2700);
  });

  it("parity: 500 consecutive calls → always 0x2700", () => {
    let ok = 0;
    for (let i = 0; i < 500; i++) {
  ...
  it("result is idempotent (no side effect)", () => {
    const r1 = disableInterrupts10110();
    const r2 = disableInterrupts10110();
    expect(r1).toBe(r2);
  });

  it("SR_IPL7_SUPERVISOR: bit 13 (supervisor) and bits 10-8 (IPL=7) set", () => {
