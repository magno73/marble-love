...
describe("stateSub2BDA (FUN_2BDA)", () => {
  it("does not raise exceptions with empty state and returns 1 (slot 0 free)", () => {
  ...
  it("registers in the first empty slot, skipping those occupied", () => {
    const s = emptyGameState();
    // Slot 0 and 1 occupied (state != 0); slot 2 and 3 empty.
  ...
  it("returns 0 without changes if all the slots are occupied", () => {
    const s = emptyGameState();
    for (let i = 0; i < 4; i++) s.workRam[STATE_BASE + i] = 1;
    // Pre-populate the struct to verify invariance
  ...
  it("uses only the low word of arg2 and arg3 (mask 0xFFFF)", () => {
    ...
    // as word: the top 16 bits are ignored.
  ...
  it("zeroes COUNTER and FLAG34 of the allocated slot even if they were dirty", () => {
    const s = emptyGameState();
