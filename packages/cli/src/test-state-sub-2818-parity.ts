#!/usr/bin/env node
/**
 * test-state-sub-2818-parity.ts — differential FUN_2818 vs stateSub2818.
 *
 * FUN_2818 is the state-machine state 6 alpha-string backward shifter.
...
import { bus as busNs, state as stateNs, stateSub2818 as sub2818Ns } from "@marble-love/engine";
import { callFunction, createCpu, disposeCpu, peekMem, pokeMem } from "./binary-oracle-lib.js";

const FUN_2818 = 0x00002818;
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < n; i++) {
    ...
    callFunction(cpu, FUN_2818, [STRUCT]);
    sub2818Ns.stateSub2818(stateInst, tsRom, STRUCT);
