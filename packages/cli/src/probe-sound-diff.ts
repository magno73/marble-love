/**
 * probe-sound-diff.ts - differential testing for TS SoundChip vs MAME oracle.
 *
 * Loads a MAME dump generated by `oracle/mame_sound_dump.lua` (JSON with audio
 * CPU regs, audioRam, YM2151 regs, POKEY regs, and mailbox state), runs the TS
 * SoundChip for N frames from reset, then byte-diffs the register shadow.
 *
 * Phase 5/6 V2 register-state parity criterion: 0 byte diff across every
 * region while the 6502 runs freely with no main CPU command input.
 *
 * Usage:
 ...
 *   Status: PASS | FAIL (criterion <= 0 byte for V2 strict)
 *
 * Phase 7 (V3 sample-level audio): add sample-stream diffing, which requires
 * envelope and FM operator sync with MAME ym2151_device::sound_stream_update.
 */
...
function hex2bytes(hex: string, expectedLen: number): Uint8Array {
  if (hex.length !== expectedLen * 2) {
    throw new Error(`hex2bytes: expected ${expectedLen * 2} chars, got ${hex.length}`);
  }
...
function main(): void {
  ...
  // TS SoundChip starts in reset hold, like the hardware. Main 68K releases it
  // around f100-200 (empirically verified via mame_sound_reset_tap). Env override:
  //   SOUND_RESET_RELEASE_FRAME=<N>   default 0 (no hold, immediately free)
  const chip = createSoundChip({ roms: { rom421, rom422 } });
  const releaseFrame = parseInt(process.env.SOUND_RESET_RELEASE_FRAME ?? "245", 10);
  if (releaseFrame > 0) {
    // Fake ticks up to releaseFrame; inReset=true consumes no cycles.
    console.log(`Reset hold ${releaseFrame} frame, then release...`);
    releaseSoundReset(chip);  // Simulates main write $860001 bit 7.
    // From releaseFrame onward, use real ticks.
    ...
    // Legacy: no hold, released immediately.
