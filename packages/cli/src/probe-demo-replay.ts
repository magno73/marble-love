...
if (scenarioPath === undefined) {
  console.error(
    ...
    "oracle/scenarios/gameplay/level1_spawn.json [oracle/scenarios/input/demo_attract.json]",
  );
  process.exit(2);
}
...
function runScenario(
  ...
  replay: inputReplay.InputReplay,
  phase: number,
): FrameDiff[] {
  ...
  for (let i = 1; i < scenario.snapshots.length; i++) {
    ...
    tick(s, { rom, runMainLoopBody: true, ...replay.mainTickInputs(target.frame) });
...
function diffEntries(
  ...
  seed: Uint8Array,
  start = 0,
  length = Math.min(ts.length, mame.length) - start,
): DiffEntry[] {
  const out: DiffEntry[] = [];
  const end = start + length;
  for (let i = start; i < end; i++) {
...
function phaseScore(diffs: FrameDiff[]): number {
  return diffs.slice(0, 10).reduce(
    (acc, d) => acc + d.playfield * 100 + d.sprite * 4 + d.hud * 4 + d.alpha + d.color + d.work,
    0,
  );
}
...
function summarizeRange(diffs: FrameDiff[], from: number, to: number): {
  max: Omit<FrameDiff, "frameIndex" | "absoluteFrame">;
  sum: Omit<FrameDiff, "frameIndex" | "absoluteFrame">;
} {
  const max = { playfield: 0, sprite: 0, hud: 0, alpha: 0, color: 0, work: 0 };
  const sum = { playfield: 0, sprite: 0, hud: 0, alpha: 0, color: 0, work: 0 };
  for (const d of diffs) {
...
function rangeStatus(max: Omit<FrameDiff, "frameIndex" | "absoluteFrame">): string {
  if (max.playfield === 0 && max.sprite === 0 && max.hud === 0 && max.alpha === 0 && max.color === 0) {
    return "BIT-PERFECT";
  }
  if (max.playfield === 0 && max.sprite <= 50 && max.hud <= 30) {
    return "PASS (under threshold)";
  }
  if (max.playfield === 0 && max.sprite <= 120 && max.hud <= 80) {
...
const trace = inputReplay.parseInputReplayTrace(readFileSync(resolve(inputPath), "utf-8"));
const replay = inputReplay.createInputReplay(trace);
const runs = phaseCandidates.map((phase) => ({ phase, diffs: runScenario(scenario, replay, phase) }));
...
for (const [from, to] of [[1, 10], [11, 30], [31, 60], [61, 100]] as const) {
  ...
  console.log(
    ...
    `sum PF=${s.playfield} Sprite=${s.sprite} HUD=${s.hud} | Status: ${rangeStatus(m)}`,
  );
}
...
if (process.env.SHOW_DIFFS !== undefined) {
  ...
  const entries = collectDiffEntries(scenario, replay, best.phase, focus);
