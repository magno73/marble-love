...
function scriptSlotField(off: number): string | null {
  // 25 slot × 0x56 byte da 0xA9C a 0xA9C + 25*0x56 = 0xA9C + 0x86A = 0x1306
  const base = 0xa9c;
  const stride = 0x56;
  const slotCount = 25;
...
function identifyField(off: number): string {
  ...
  // 6) decode buffer 0x700..0x77F (74B noto)
...
console.log(`Total gameplay diff: ${rows.length} byte (stack residue ${totW - totGameplay} esclusi)\n`);
...
mdLines.push("");
mdLines.push(`Generato da \`packages/cli/src/probe-gameplay-byte-map.ts\`.`);
mdLines.push("");
