 ...
 * `alphaRamBootInitED6`.
 *
 *   - 30 rows × 84 bytes (loop 1, ROM-driven)
 *   - 34 words at offset 0x008..0x04B (loop 2, constant 0x2000)
 *   - 34 words at offset 0xE88..0xECB (loop 3, constant 0x2000)
 * The rest of the alpha RAM (44 "skip" bytes per row + 0xF00..0xFFF)
 * stays as it was.
 *
 * Test strategy:
 *     1. Pre-fill alpha RAM [0xA03000..0xA04000) with a random pattern
 *        (synchronized between binary and TS).
 *
 *
 * Usage: npx tsx packages/cli/src/test-alpha-ram-boot-init-ed6-parity.ts [N]
 */
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // ─── Setup: pre-fill alpha RAM on both sides ─────────────────────
    for (let j = 0; j < ALPHA_RAM_SIZE; j++) {
    ...
    // ─── Run binary ──────────────────────────────────────────────────────
    // FUN_ED6 has 30×42 + 34 + 34 = ~1330 word writes ⇒ ~50-60k cycles.
