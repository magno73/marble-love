#!/usr/bin/env node
/**
 * Differential parity for FUN_28580 vs initFnPointers28580.
 *
 * Internal FUN_014E is patched to RTS and mirrored by the TS default no-op.
 */
...
import { bus as busNs, initFnPointers28580 as initNs, state as stateNs } from "@marble-love/engine";
import { callFunction, createCpu, disposeCpu, peekMem, pokeMem } from "./binary-oracle-lib.js";

const FUN_28580 = 0x00028580;
const FUN_014E = 0x0000014e;
const WORK_BASE = 0x00400000;
const COMPARE_OFFS = [0x412, 0x41e, 0x42a, 0x436] as const;
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (const off of COMPARE_OFFS) {
      for (let j = 0; j < 4; j++) {
        const v = Math.floor(rng() * 256) & 0xff;
        pokeMem(cpu, WORK_BASE + off + j, 1, v);
        tsState.workRam[off + j] = v;
      }
    }

    callFunction(cpu, FUN_28580, []);
    initNs.initFnPointers28580(tsState, tsRom);
